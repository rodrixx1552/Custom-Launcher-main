...
// --- MÓDULO JARVIS VOICE COMMANDER (STT) 🎤🤖 ---
class JarvisVoice {
    constructor() {
    ...
    init() {
        ...
        if (!SpeechRecognition) return console.warn('JARVIS: STT Not Supported.');
        ...
        this.recognition.onstart = () => { this.isListening = true; console.log('JARVIS: Oído activo.'); };
        this.recognition.onresult = (event) => {
            if (this.isProcessing) return;
            for (let i = event.resultIndex; i < event.results.length; ++i) {
                if (event.results[i].isFinal) {
                    this.processCommand(event.results[i][0].transcript.toLowerCase().trim());
                }
            }
        };
        this.recognition.onerror = () => { this.isListening = false; setTimeout(() => this.start(), 3000); };
        this.recognition.onend = () => { this.isListening = false; this.start(); };
        this.start();
    }

    start() { if (!this.isListening) try { this.recognition.start(); } catch(e) {} }

    async processCommand(text) {
        if (!text.includes(this.wakeWord)) return;
        this.isProcessing = true;
        const cmd = text.split(this.wakeWord)[1]?.trim() || "";
        
        // Visual Pulse
        const p = document.createElement('div');
        p.style.cssText = 'position:fixed;bottom:20px;right:20px;width:12px;height:12px;background:#ffb7c5;border-radius:50%;box-shadow:0 0 15px #ffb7c5;z-index:9999;animation:pulse 1s infinite;';
        document.body.appendChild(p);
        setTimeout(() => p.remove(), 3000);

        if (cmd.includes("enciende") || cmd.includes("prende") || cmd.includes("servidor") || cmd.includes("server")) {
            await window.speak("Protocolo de arranque Aternos iniciado. Preparando acceso.");
            window.electronAPI.autoStartServer();
        } else if (cmd.includes("jugar") || cmd.includes("inicia") || cmd.includes("abre minecraft")) {
            const b = document.getElementById('play-btn');
            if (b && !b.disabled) { await window.speak("Iniciando motores. Buena suerte, señor."); b.click(); }
            else await window.speak("Sistemas de lanzamiento bloqueados. Revise su cuenta.");
        } else if (cmd.includes("skins") || cmd.includes("apariencia")) {
            await window.speak("Accediendo a la bóveda de apariencias.");
            document.querySelector('.nav-item[onclick*="Skins"]')?.click() || (window.renderSkinsTab && window.renderSkinsTab());
        } else if (cmd.includes("ajustes") || cmd.includes("configuración")) {
            await window.speak("Cargando parámetros del centro de mando.");
            document.querySelector('.nav-item[onclick*="Settings"]')?.click() || (window.renderSettingsTab && window.renderSettingsTab());
        } else if (cmd.includes("hola") || cmd.includes("saluda")) {
            await window.speak("Hola señor Rodri. Estoy a su servicio.");
        }
...
window.JarvisSTT = new JarvisVoice();
window.JarvisSTT.init();
