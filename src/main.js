...
const createWindow = () => {
    ...
    // Multi-background logic removed (Assets purged). Using high-quality fixed background.
    ejse.data('bgId', path.join(__dirname, 'assets', 'los_papus', 'background.png')) // Get background based on what background is set in settings.json
...
// Aternos Automation Removed (HolyHosting Migration)
...
ipcMain.handle('get-server-ip', async () => {
    // 1. Check Local Config First (Priority for development/immediate changes)
    try {
    ...
    try {
        ...
        if (config && config.server) return `${config.server.ip}:${config.server.port}`;
        if (config && config.server_ip && !config.server_ip.includes('aternos')) return config.server_ip;
    } catch (e) {
